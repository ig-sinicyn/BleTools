...
namespace BleSend.Temp
{
    internal class ProgramOld
    {
        ...
        public static async Task Mai2n(string[] args)
        {
            ...
            Console.WriteLine(device.ConnectionStatus);
            ...
            var gattDeviceServices = (await device.GetGattServicesForUuidAsync(_serviceId, BluetoothCacheMode.Uncached)).Services;

            using var svc = device.GattServices.First(x => x.Uuid == _serviceId);
            svc.Session.MaintainConnection = true;

            //var ch = (await svc.GetCharacteristicsForUuidAsync(_charId, BluetoothCacheMode.Uncached)).Characteristics.Single();
        ...
        private static async Task<DeviceInformation> FindDeviceAsync(string deviceId)
        {
